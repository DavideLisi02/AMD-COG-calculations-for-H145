...
#   Set up the objects
h145_zf = Object("H145_ZF", H145_to_mass, H145_to_BL, H145_to_STA)
...
#   Calculate the CoG
cog_2 = [h145_zf, rail_2, box_2, support_2]
...
#   Calculate the CoG
cog_3 = [h145_zf, rail_3, box_3, support_3, patient_3]
...
#   Calculate the CoG
cog_4 = [h145_zf, rail_4, box_4, support_4, patient_4]
...
#   Calculate the CoG
cog_5 = [h145_zf, rail_5, box_5, support_5, patient_5]
...
patient_6 = Object("Patient_6", human_mass, -420, 4010)
...
#   Calculate the CoG
cog_6 = [h145_zf, rail_6, box_6, support_6, patient_6]
...
def update(frame):
    ...
    # Interpolate COG for current frame
    num_frames_per_step = 20
    step = frame // num_frames_per_step
    if step < len(cog_coords) - 1:
        t = (frame % num_frames_per_step) / num_frames_per_step
        (cog_sta1, cog_bl1), total_mass1 = cog_coords[step]
        (cog_sta2, cog_bl2), total_mass2 = cog_coords[step+1]
        
        cog_sta = cog_sta1 + (cog_sta2 - cog_sta1) * t
        cog_bl = cog_bl1 + (cog_bl2 - cog_bl1) * t
        total_mass = total_mass1 + (total_mass2-total_mass1) * t
    else:
        (cog_sta, cog_bl), total_mass = cog_coords[-1]

    
    ax1.scatter(cog_sta, total_mass, color='red', s=50)
    ax2.scatter(cog_bl, total_mass, color='red', s=50)


    return ax1, ax2

# Create the animation
ani = animation.FuncAnimation(fig, update, frames=len(cog_coords)*20, repeat=True, interval=50)
